...
contract CErc20Mock is ReentrancyGuard, TokenErrorReporter, ERC20 {
     ...
     */
    constructor(address underlying_, string memory name_, string memory symbol_, uint256 decimals_) public {
        name = name_;
        symbol = symbol_;
        decimals = decimals_;
        // Set underlying
    ...
    function mint(uint256 mintAmount) external returns (uint256) {
        uint256 tokenExpValue = getExpTokenValue();

        uint256 mintedTokens = (mintAmount.mul(1e18)).div(tokenExpValue);

        _mint(msg.sender, mintedTokens);

        return uint256(doTransferIn(msg.sender, mintAmount));
    }

    function supplyUnderlying(uint256 supplyAmount) external returns (uint256) {
        return uint256(doTransferIn(msg.sender, supplyAmount));
    }
    ...
    function redeemUnderlying(uint256 redeemAmount) external returns (uint256) {
        ...
        uint256 redeemTokens = redeemAmount.mul(1e18).div(tokenExpValue);

        _burn(msg.sender, redeemTokens);

        return uint256(doTransferOut(msg.sender, redeemAmount));
    }
    ...
    function getExpTokenValue() public view returns (uint256) {
        uint256 totalUnderlying = EIP20Interface(underlying).balanceOf(address(this));
        uint256 tokenValueExp = 1e18;
